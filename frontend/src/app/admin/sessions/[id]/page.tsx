...
import { sessionsApi, workoutLogsApi, programsApi, membersApi } from '@/lib/api';
...
function intensityBar(rpe: string, rir: string) {
  ...
  const color = effectiveRpe >= 9 ? 'bg-red-500' : effectiveRpe >= 8 ? 'bg-orange-400' : effectiveRpe >= 7 ? 'bg-yellow-400' : 'bg-green-400';
  return (
...
const EMPTY_ROW = (dayTitle = ''): LogRow => ({
  dayTitle, exerciseName: '', weight: '', sets: '3', reps: '10',
  rir: '2', rpe: '8', notes: '', saved: false, fromProgram: false,
});
...
export default function SessionLogPage() {
  ...
  const load = useCallback(async () => {
    ...
    if (savedLogs.length > 0) {
      setRows(savedLogs.map(l => ({
        ...
        rir: String(l.rir ?? ''), rpe: String(l.rpe ?? ''), notes: l.notes || '',
        saved: true, fromProgram: false,
      })));
    }

    // Load member's programs for picker
    if (s.memberId) {
      const progs: Program[] = await programsApi.listByMember(s.memberId).catch(() => []);
      setPrograms(progs);
      ...
      if (savedLogs.length === 0 && s.programId) {
        ...
        if (prefilled.length > 0) {
          setRows(prefilled.map((p: any) => ({
            ...
            weight: String(p.weight || ''), sets: String(p.sets || 3),
            reps: String(p.reps || 10), rir: String(p.rir ?? 2),
            rpe: String(p.rpe ?? 8), notes: p.notes || '',
            saved: false, fromProgram: true,
          })));
        }
      }
    }
  ...
  useEffect(() => {
    ...
    if (session?.endedAt) {
      setElapsed(dayjs(session.endedAt).diff(dayjs(session.startedAt), 'second'));
      return;
    }
  ...
  // ── Save ──
  const saveAll = async () => {
    const invalid = rows.find(r => !r.exerciseName.trim() || !r.weight || !r.sets || !r.reps);
    if (invalid) return toast.error(`กรอกข้อมูลให้ครบ: ชื่อท่า, น้ำหนัก, เซ็ต, ครั้ง`);
    setSaving(true);
    try {
  ...
  const endSession = async () => {
    if (rows.some(r => !r.saved)) {
      if (!confirm('ยังมีข้อมูลที่ยังไม่ได้บันทึก ต้องการจบ Session หรือไม่?')) return;
    }
    ...
    setSession(prev => prev ? { ...prev, status: 'completed', endedAt: new Date().toISOString() } : prev);
  ...
  // ── Stats ──
  const totalVolume = rows.reduce((sum, r) => sum + calcVol(r.weight, r.sets, r.reps), 0);
  const totalSets = rows.reduce((s, r) => s + (+r.sets || 0), 0);
  const avgRpe = rows.filter(r => r.rpe).length
    ? rows.filter(r => r.rpe).reduce((s, r) => s + +r.rpe, 0) / rows.filter(r => r.rpe).length
    : 0;

  // Group rows by dayTitle for display
  const groupedDays = [...new Set(rows.map(r => r.dayTitle || '—'))];
    ...
    <div className="animate-fade-in space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3">
          <Link href="/admin/sessions" className="btn-ghost btn-sm">← กลับ</Link>
          <div>
            <h1 className="page-title">Workout Log</h1>
            <div className="flex items-center gap-2 mt-0.5 text-sm text-gray-500">
              <span>👤 {session.member?.name}</span>
              <span>•</span>
              <span>📅 {dayjs(session.scheduledAt).format('ddd DD/MM/YYYY HH:mm')}</span>
              <span>•</span>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: 'ท่าทั้งหมด', value: rows.length, unit: 'ท่า', color: 'text-gray-900' },
          { label: 'Total Sets', value: totalSets, unit: 'set', color: 'text-blue-700' },
          { label: 'Total Volume', value: totalVolume.toLocaleString(), unit: 'kg', color: 'text-purple-700' },
          { label: 'RPE เฉลี่ย', value: avgRpe ? avgRpe.toFixed(1) : '-', unit: '/ 10', color: avgRpe >= 8.5 ? 'text-red-600' : 'text-orange-500' },
        ].map(s => (
      ...
      <div className="card p-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-sm font-semibold text-gray-700">📋 โหลดจากโปรแกรม</span>
            {selectedDay && (
          ...
          <div className="mt-4 animate-fade-in">
            {programs.length === 0 ? (
              <div className="text-center py-6 text-gray-400 text-sm">
                ...
                <Link href="/admin/programs" className="text-blue-500 underline">สร้างโปรแกรม</Link>
              </div>
            ) : (
                  ...
                  <div key={prog.id} className="border border-gray-200 rounded-xl overflow-hidden">
                    ...
                    <div
                      className={`px-4 py-2.5 flex items-center justify-between cursor-pointer transition-colors ${
                        selectedProgram?.id === prog.id ? 'bg-blue-50' : 'bg-gray-50 hover:bg-gray-100'
                      }`}
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        {prog.isActive && <span className="badge-green text-xs">Active</span>}
                        <span className="text-xs text-gray-400">{prog.days?.length || 0} วัน</span>
                      </div>
                      <span className="text-gray-400 text-xs">{selectedProgram?.id === prog.id ? '▲' : '▼'}</span>
                    </div>

                    {/* Days */}
                    {selectedProgram?.id === prog.id && (
                      <div className="p-3 grid grid-cols-2 md:grid-cols-3 gap-2 bg-white">
                        {prog.days?.sort((a, b) => a.dayOrder - b.dayOrder).map(day => (
                          <button
                            key={day.id}
                            onClick={() => { setSelectedDay(day); loadFromProgramDay(day); }}
      ...
      <div className="card overflow-hidden">
        <div className="card-header">
          ...
          <button onClick={() => addRow(groupedDays[0] || '')} className="btn-secondary btn-sm">
            + เพิ่มท่า
          </button>
        </div>
        ...
        {rows.length === 0 ? (
          <div className="py-16 flex flex-col items-center gap-4 text-gray-400">
            ...
            <div className="text-center">
              ...
              <div className="text-sm mt-1">เลือก <strong>โหลดจากโปรแกรม</strong> หรือเพิ่มเองด้านบน</div>
            </div>
            <button onClick={() => setShowProgramPicker(true)} className="btn-primary btn-sm">
              📋 โหลดจากโปรแกรม
            </button>
          </div>
        ) : (
            ...
            <table className="w-full text-sm min-w-[900px]">
              <thead>
                <tr className="bg-gray-50 border-b border-gray-100">
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-400 uppercase w-6">#</th>
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-400 uppercase w-28">กลุ่มกล้าม</th>
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-400 uppercase">ท่าออกกำลังกาย</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase w-24">น้ำหนัก (kg)</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase w-16">Set</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase w-16">Reps</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase w-20">Volume</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase w-16">RIR</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase w-16">RPE</th>
                  <th className="px-3 py-3 text-center text-xs font-semibold text-gray-400 uppercase w-24">ความเข้มข้น</th>
                  <th className="px-3 py-3 text-left text-xs font-semibold text-gray-400 uppercase">หมายเหตุ</th>
              ...
              <tbody>
                {rows.map((row, i) => {
                  ...
                  // Day divider
                  const showDivider = i === 0 || row.dayTitle !== rows[i - 1].dayTitle;
                  return (
                    <>
                      {showDivider && row.dayTitle && (
                        <tr key={`div-${i}`} className="bg-gradient-to-r from-blue-50 to-indigo-50">
                          <td colSpan={12} className="px-4 py-2">
                            <div className="flex items-center gap-2">
                              <span className="font-bold text-blue-700 text-sm">{row.dayTitle}</span>
                      ...
                      <tr
                        ...
                        className={`border-b border-gray-50 transition-colors ${
                          row.saved ? 'bg-emerald-50/30' : row.fromProgram ? 'bg-blue-50/20' : 'bg-white hover:bg-gray-50'
                        }`}
                      >
                        ...
                        <td className="px-3 py-2.5">
                          <input
                            ...
                            value={row.dayTitle}
                            onChange={e => updateRow(i, 'dayTitle', e.target.value)}
                          />
                        </td>
                        <td className="px-3 py-2.5">
                          <input
                        ...
                        <td className="px-3 py-2.5">
                          <div className="flex gap-1">
                            <button onClick={() => moveRow(i, -1)} disabled={i === 0}
                              className="w-6 h-6 flex items-center justify-center text-gray-300 hover:text-gray-600 disabled:opacity-20 text-xs">↑</button>
                            <button onClick={() => moveRow(i, 1)} disabled={i === rows.length - 1}
                              className="w-6 h-6 flex items-center justify-center text-gray-300 hover:text-gray-600 disabled:opacity-20 text-xs">↓</button>
                            <button onClick={() => removeRow(i)}
                              className="w-6 h-6 flex items-center justify-center text-red-300 hover:text-red-600 text-sm">×</button>
                          </div>
                        </td>
                      </tr>
                    </>
              ...
              {rows.length > 0 && (
                <tfoot>
                  <tr className="bg-gray-50 border-t-2 border-gray-200">
                    <td colSpan={4} className="px-4 py-3 text-xs font-semibold text-gray-500 uppercase">รวมทั้งหมด</td>
                    <td className="px-3 py-3 text-center font-bold text-blue-700">{totalSets}</td>
                    <td />
                    ...
                    <td className="px-3 py-3 text-center">
                      {avgRpe > 0 && (
                        <span className={`text-xs font-bold ${rpeColor(String(avgRpe))}`}>
      ...
      {rows.length > 0 && (
        <div className="flex flex-wrap gap-2">
          ...
          {groupedDays.map(day => (
            <button key={day} onClick={() => addRow(day === '—' ? '' : day)}
              className="btn-ghost btn-sm text-xs border border-gray-200">
              + {day}
            </button>
          ))}
          <button onClick={() => addRow()} className="btn-ghost btn-sm text-xs border border-dashed border-gray-300">
            + กลุ่มใหม่
          </button>
        </div>
      )}
