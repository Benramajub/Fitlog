...
export const sessionsApi = {
  ...
  schedule: (data: any) =>
    api.post("/sessions/schedule", data).then((r) => r.data),
...
export const nutritionApi = {
  calculate: (data: any) =>
    api.post("/nutrition/calculate", data).then((r) => r.data),
  createPlan: (data: any) =>
  ...
  getChart: (memberId: string, days = 30) =>
    api
      .get(`/nutrition/chart/${memberId}`, { params: { days } })
      .then((r) => r.data),
  ...
  getLogs: (memberId: string, from: string, to: string) =>
    api
      .get(`/nutrition/logs/${memberId}`, { params: { from, to } })
      .then((r) => r.data),
};
...
export const workoutLogsApi = {
  ...
  getPrefilledFromDay: (dayId: string) =>
    api.get(`/workout-logs/prefilled-day/${dayId}`).then((r) => r.data),
  saveBulk: (sessionId: string, logs: any[]) =>
    api
      .post(`/workout-logs/session/${sessionId}/bulk`, { logs })
      .then((r) => r.data),
  update: (id: string, data: any) =>
    api.patch(`/workout-logs/${id}`, data).then((r) => r.data),
  remove: (id: string) => api.delete(`/workout-logs/${id}`).then((r) => r.data),
  history: (memberId: string, exercise?: string) =>
    api
      .get(`/workout-logs/history/${memberId}`, { params: { exercise } })
      .then((r) => r.data),
};
