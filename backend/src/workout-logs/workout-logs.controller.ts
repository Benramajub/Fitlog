...
export class WorkoutLogsController {
  ...
  getForSession(@Param('sessionId') id: string) { return this.s.getLogsForSession(id); }

  @Get('session/:sessionId/prefilled')
  getPrefilled(@Param('sessionId') id: string) { return this.s.getPrefilled(id); }

  @Get('history/:memberId')
  getHistory(@Param('memberId') memberId: string, @Query('exercise') exercise?: string) {
  ...
  @Roles('admin')
  saveBulk(@Param('sessionId') sessionId: string, @Body() body: { logs: WorkoutLogDto[] }) {
  ...
  @Delete(':id')
  @Roles('admin')
  remove(@Param('id') id: string) { return this.s.deleteLog(id); }
}
